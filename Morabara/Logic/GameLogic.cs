...
namespace Morabara.Logic
{
    public class GameLogic
    {
        ...
        private void setFields(List<Field> f)
        {
            this.fields = f;
        }
        ...
        // GLOBAL
        /*
        public static List<Neighborhood> neighs = new List<Neighborhood>
            {
        ...
        class AlphaBeta_Core
        {
            ...
            public AlphaBeta_Core(List<int[]> values, bool turnForPlayerX)
            {
            ...
            public IEnumerable<AlphaBeta_Core> GetSubtrees() 
            {
            ...
            public int AlphaBetaPruning(int depth, bool needMax, int alpha, int beta, out AlphaBeta_Core childWithMax)
            {
                ...
                foreach (AlphaBeta_Core core in GetSubtrees())
                {
                    AlphaBeta_Core foo;
            ...
            void GetScore() //przeliczanie punktów
            {
            ...
            {
               List<int> tmpAvailableMoves = CheckMovesFirstStage(turn, board); 
        ...
        private void PlaceComputerBall()
        {
            ...
            Task firstStage = new Task(() =>
            {
                ...
                List<int[]> fieldsForMiniMax = new List<int[]>();
                foreach (var field in fields)
                {
                    int takenBy;
                    if (field.TakenBy == TakenBy.Computer)
                        takenBy = 2;
                    else if (field.TakenBy == TakenBy.Player)
                        takenBy = 1;
                    else
                        takenBy = 0;

                    fieldsForMiniMax.Add(new int[] { field.Id, takenBy });
                }

                AlphaBeta_Core Current = new AlphaBeta_Core(fieldsForMiniMax, false); //budowanie stanu maszyny na podstawie aktualnego stanu planszy

                AlphaBeta_Core next = Current.FindNextMove(depth); //szukanie najlepszego ruchu
        ...
        private void MoveComputerBall()
        {
            Task secondStage = new Task(() =>
            {
                ...
                /*AlphaBeta_Core Current = new AlphaBeta_Core(fields, false); //budowanie stanu maszyny na podstawie aktualnego stanu planszy
                AlphaBeta_Core next = Current.FindNextMove(depth); //szukanie najlepszego ruchu
                if (next != null)
                {
                    MoveBallOrder minimaxMove = new MoveBallOrder
                    {
                        IdFieldFrom = Convert.ToInt32(next.Moves[0]),
                        IdFieldTo = next.Moves[1]
                    };
                    //moveBallOrder.IdFieldFrom = Convert.ToInt32(next.Moves[0]);
                    //moveBallOrder.IdFieldTo = next.Moves[1];

                    moveBallOrder = minimaxMove;
                ...
                {
                    Debug.WriteLine("Moving ball BLABLABLA");
                    Thread.Sleep(750);
